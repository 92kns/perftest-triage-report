...
// requestSlots bounds total in-flight Treeherder requests across every phase.
//
// Each analyzer used to build its own maxConcurrent-sized semaphore, so running
// them in parallel allowed ~3x maxConcurrent simultaneous requests and provoked
// client timeouts against Treeherder. One shared limiter makes --concurrency an
// actual ceiling.
//
// Safe against deadlock only because no code path acquires a slot while already
// holding one: the fan-out goroutines acquire, do leaf HTTP work, and release.
var requestSlots chan struct{}

// initConcurrency sizes the shared limiter. n must be >= 1.
func initConcurrency(n int) {
	maxConcurrent = n
	requestSlots = make(chan struct{}, n)
}

func acquire() {
	// Tests that call analyzers directly may not have run initConcurrency.
	if requestSlots == nil {
		return
	}
	requestSlots <- struct{}{}
...
func main() {
	...
	noSuites := flag.Bool("no-suites", false, "Skip the Test Suites tab")
	noRates := flag.Bool("no-rates", false, "Skip the per-job success rate sweep (the slowest phase)")
	discover := flag.Bool("discover-job-groups", false, "Scan recent jobs for perf job groups missing from perfJobGroups, then exit")
	excludeAndroid := flag.Bool("rates-exclude-android", false, "Exclude android job types from the Job Success Rate tab (matches the Redash dashboard)")
	...
	flag.IntVar(&topN, "top", 25, "Max rows in the Test Suites and Job Success Rate tabs")
	flag.Parse()

	if *concurrency < 1 {
		// A zero-capacity limiter blocks forever; a negative one panics in make.
		log.Fatalf("--concurrency must be at least 1, got %d", *concurrency)
	}
	if daysBack < 1 {
		log.Fatalf("--days must be at least 1, got %d", daysBack)
	}
	...
	if *discover {
		fmt.Println("Scanning recent jobs for unlisted perf job groups...")
		missing := discoverJobGroups(startDay, endDay)
		if len(missing) == 0 {
			fmt.Println("✅ No new perf job groups found; perfJobGroups is up to date.")
			return
		}
		fmt.Printf("⚠️  %d perf job group(s) missing from perfJobGroups in jobs.go:\n", len(missing))
	...
	if !*noSuites {
		wg.Add(1)
		go func() { defer wg.Done(); suiteBugs = fetchSuiteBugs() }()
	}
	if !*noRates {
		wg.Add(1)
		go func() { defer wg.Done(); jobStats, sweep = fetchJobStats(startDay, endDay) }()
	}
	wg.Wait()
	...
	if !*noSuites {
		...
		go func() {
			defer wg2.Done()
			suites = analyzeSuites(suiteBugs, startDay, endDay, twoDayStart)
		}()
	}
	wg2.Wait()

	if len(suites) > 0 && len(jobStats) > 0 {
		matched := joinSuiteRates(suites, suiteRatesFrom(jobStats))
		fmt.Printf("Joined per-job success rates onto %d/%d suites\n", matched, len(suites))
	}

	if len(results) == 0 && len(permas) == 0 && len(suites) == 0 && len(jobStats) == 0 {
		fmt.Println("No matching bugs found.")
		return
	}

	writeHTMLReport(reportInput{
		Results:     results,
		Permas:      permas,
		TaskTimeout: taskTimeout,
		Harnesses:   groupByHarness(suites),
		JobStats:    filterJobStats(jobStats, jobRateMinRuns, topN, *excludeAndroid),
		RateWarning: sweep.Warning(),
	})
...
// fetchBugList runs one Bugzilla query and returns its bugs. Unlike
// fetchIntermittentBugs it logs and returns nil on failure rather than exiting,
// so a Bugzilla hiccup degrades one tab instead of killing the whole report.
...
// jobRateMinRuns is the minimum number of runs a job type needs before its
// success rate is worth showing. Below this a single failure swings the
// percentage wildly (1 fail in 3 runs reads as a dramatic 66%).
const jobRateMinRuns = 20

// reportInput is what main() hands to the renderer. It is a struct rather than
// a positional argument list so tabs can be added without churning callers.
...
func writeHTMLReport(in reportInput) {
	tmpl := reportTemplate

	data := reportData{
		Intermittents: groupByComponent(in.Results, components),
		Permas:        groupByComponent(in.Permas, components),
		TaskTimeout:   in.TaskTimeout,
		Harnesses:     in.Harnesses,
		JobStats:      in.JobStats,
		RateWarning:   in.RateWarning,
		Generated:     time.Now().UTC().Format("2006-01-02 15:04 MST"),
		DaysBack:      daysBack,
		TopN:          topN,
		MinRuns:       jobRateMinRuns,
	}
...
// templateFuncs are helpers the report template needs. Kept tiny on purpose --
// anything with real logic belongs in Go where it can be tested.
var templateFuncs = template.FuncMap{
	// ratePctClass buckets a success rate for colouring: red below 90%,
	// amber below 99%, unstyled above.
	"ratePctClass": func(success, total int) string {
		if total == 0 {
			return ""
		}
		switch {
		case success*100 < total*90:
			return "pct-bad"
		case success*100 < total*99:
			return "pct-warn"
		default:
			return ""
		}
...
func renderHTML(w io.Writer, tmpl string, data any) error {
	t := template.Must(template.New("report").Funcs(templateFuncs).Parse(tmpl))
